...
export class QuizService {
  ...
  getQuizContent(categoryId: number) {
    ...
    this.http.get('http://localhost:3000/questions').subscribe((questions: any) => {
      const questionsByCategorie = questions.filter((question: any) => question.categoryId == categoryId);
      console.log("elcisssss   ",questionsByCategorie)
      for (const question of questionsByCategorie) {
